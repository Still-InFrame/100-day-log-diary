...
- **Deploys are manual; GitHub↔Vercel not yet linked.** Code is on GitHub (`git@github.com:Still-InFrame/100-day-log-diary.git`, default branch `main`, pushed over SSH key `~/.ssh/id_ed25519`). But the Vercel project is NOT connected to the repo, so `git push` does NOT auto-deploy — deploys are still a manual `npx ... vercel@latest --prod` from the project dir. To get push-to-deploy, connect the repo in Vercel dashboard → Project → Settings → Git (parked). Always `git push` AND `vercel --prod` until then.
...
- **Project folder name has spaces and a capital ("Day 1 - Log Diary").** `create-next-app` rejects the folder name as an npm package name ("name can only contain URL-friendly characters; can no longer contain capital letters"). Workaround used: scaffolded to `/tmp/log-diary-scaffold`, copied contents back with `cp -R /tmp/log-diary-scaffold/. ./`, renamed `package.json` `name` field to `log-diary`. If re-scaffolding ever needed, repeat that flow — do not try to scaffold directly into this folder.
