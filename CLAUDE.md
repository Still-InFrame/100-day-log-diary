...
- **"Current streak: 0" shows next to the completion banner post-Day-100.** After the challenge completes, `computeStreaks` still computes `current` live against today's date; since the last log (Day 100) is now in the past, current reads 0 — awkward under a "100 days straight" celebration. Left as-is 2026-09-10 pending Savion's decision (options floated: show the final `100`, or a "Complete ✓" label, when `totalLogged >= TOTAL_DAYS`). Change would live in [components/StreakBanner.tsx](components/StreakBanner.tsx) + the profile `Stat` cards, gated on the same `complete` flag the banner uses.
