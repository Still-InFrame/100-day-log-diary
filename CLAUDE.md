...
- **Vercel deploy** — parked 2026-06-01. Reason: stretch goal. Hosted Supabase makes deploy a two-step (push to GitHub → Vercel import + env vars + add Vercel URL to OAuth redirects). Pick back up when: Savion is happy with local UX and wants to put it online.
- **Reminder notifications (email or browser)** — parked 2026-06-01 per Day 1 plan. Reason: needs hosted infra (email service like Resend, or service-worker push). Pick back up when: Savion misses a day and wants automated nudges, OR after the Vercel deploy.
